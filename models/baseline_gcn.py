...
Data = namedtuple('Data', ['x', 'edge_index', 'edge_weight'])
...
class GCN(torch.nn.Module):
    ...
    def __init__(self, config, data):
        ...
        self.x = torch.tensor(data.x).to(config['device'])
        self.edge_index = torch.tensor(data.edge_index.tolist()).to(config['device'])
    ...
    def _build(self):
        ...
        self.conv1 = GCNConv(input_dim, self.conv_dim[0], cached=True)
        # self.conv2 = GCNConv(self.conv_dim[0], self.conv_dim[1], cached=True)
    ...
    def forward(self, users, items):
        ...
        x, edge_index, edge_weight = self.x, self.edge_index, None
        ...
        if not self.mlp:
            ...
            logits = torch.sum(torch.mul(h_u, h_i), dim=1, keepdim=True)
        else:
...
def build_graph_data(mat, config):
    ...
    return Data(x=x, edge_index=edge_index, edge_weight=None)
